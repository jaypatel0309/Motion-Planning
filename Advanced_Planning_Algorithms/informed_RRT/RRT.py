...
class RRT:
    ...
    def rotation_to_world_frame(self):
        ...
        a1 = (self.goal.row-self.start.row)/c_min, (self.goal.col-self.start.col)/c_min
        ...
        d_mat = np.diag([1,np.linalg.det(U)*np.linalg.det(V)])

        C = np.dot(np.dot(U,d_mat),V.T)
        return C
